#!/usr/bin/env python3
"""
Enhanced training script for Power Grid RL on TPU with TensorBoard logging and checkpoint resume
...
class GridRLLearner(StoppableComponent):
    ...
    def _update_step(self, state: TrainState, batch: Dict) -> Tuple[TrainState, Dict]:
        ...
        # Extract observations from batch (assuming shape [1, T, B, obs_dim])
        obs_data = batch['observations'][0]  # Remove the extra batch dimension
        ...
        action_data = batch['actions'][0]  # Remove the extra batch dimension
        actions = {
        ...
        rewards_data = batch['rewards'][0]  # Remove extra dimension
        dones_data = batch['dones'][0]
        
        # Need to extract strategic values from the nested structure
        values_data = batch['values'][0] if isinstance(batch['values'], jnp.ndarray) else batch['values']
        if isinstance(values_data, dict):
            strategic_values = values_data['strategic']
        else:
            # If values are stacked, use the first set as strategic values
            strategic_values = values_data
        
        advantages, returns = compute_gae_hierarchical(
            rewards_data,
            strategic_values,  # Use strategic values as baseline
            dones_data,
    ...
    def _run(self):
        ...
        while not self.should_stop:
            ...
            if self.writer and update_count % self.config.log_frequency == 0:
...
def train_grid_rl(args):
    ...
    print("\nStarting training components...", flush=True)
    for actor in actors:
        actor.start()
    learner.start()
    
    print(f"Training with {len(actors)} actors and 1 learner", flush=True)
    print(f"Total parallel environments: {len(actors) * config.num_envs_per_actor}", flush=True)
    print(f"TensorBoard logging to: {config.tensorboard_dir}", flush=True)
    print("-" * 60, flush=True)
    ...
    try:
        ...
        # Stop all components
        print("\nStopping training components...", flush=True)
        for actor in actors:
            actor.stop()
        learner.stop()
        pipeline.stop()
        ...
        print(f"\nTraining time: {elapsed/3600:.2f} hours", flush=True)
        print("=" * 60, flush=True)
