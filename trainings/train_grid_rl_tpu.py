...
class GridRLConfig(NamedTuple):
    ...
    tensorboard_dir: str = "/home/tarive/persistent_storage/tpu_rl_workspace/grid_rl/tensorboard"
...
class GridRLActor(StoppableComponent):
    ...
    def _run(self):
        ...
        with jax.default_device(self.actor_device):
            ...
            episode_lengths = np.zeros(self.config.num_envs_per_actor, dtype=np.int32)
            episode_count = 0
            ...
            while not self.should_stop:
                ...
                for _ in range(self.config.trajectory_length):
                    ...
                    # Extract strategic value for simplicity (values is a dict)
                    strategic_value = values['strategic'] if isinstance(values, dict) else values
                    trajectory['values'].append(np.array(strategic_value))
                        ...
                        if done:
                            ...
                            if self.writer:
                                self.writer.add_scalar(
                                    f'actor_{self.actor_id}/episode_return',
                                    episode_returns[i],
                                    step_count + i
                                )
                                self.writer.add_scalar(
                                    f'actor_{self.actor_id}/episode_length',
                                    episode_lengths[i],
                                    step_count + i
                                )
                            episode_returns[i] = 0
                ...
                if step_count % (self.config.log_frequency * self.config.num_envs_per_actor) == 0:
                    mean_return = np.mean(episode_returns[episode_returns != 0]) if np.any(episode_returns != 0) else 0
                    self.log.info(
...
class GridRLLearner(StoppableComponent):
    ...
    def _update_step(self, state: TrainState, batch: Dict) -> Tuple[TrainState, Dict]:
        ...
        # Reshape batch data from [1, T, B, ...] to [T*B, ...]
        # The batch comes in with an extra dimension from pmap
        obs_data = batch['observations'][0]  # Remove pmap dimension [T, B, obs_dim]
        obs_data = obs_data.reshape(-1, obs_data.shape[-1])  # Flatten to [T*B, obs_dim]
        
        # Prepare observations
        observations = {
            'strategic': obs_data[:, :self.config.agent_config.strategic_obs_dim],
            'operational': obs_data[:, 
                          self.config.agent_config.strategic_obs_dim:
                          self.config.agent_config.strategic_obs_dim + 
                          self.config.agent_config.operational_obs_dim],
            'safety': obs_data[:, -self.config.agent_config.safety_obs_dim:]
        }
        
        # Prepare actions (need to split combined actions)
        action_data = batch['actions'][0]  # Remove pmap dimension
        action_data = action_data.reshape(-1, action_data.shape[-1])  # Flatten to [T*B, action_dim]
        
        actions = {
            'strategic': action_data[:, 0],
            'operational': [action_data[:, i+1] 
                          for i in range(self.config.agent_config.num_operational_agents)],
            'safety': action_data[:, -1]
        }
        
        # Process rewards and dones
        rewards_data = batch['rewards'][0].flatten()  # Flatten to [T*B]
        dones_data = batch['dones'][0].flatten()  # Flatten to [T*B]
        
        # Extract values - now simplified to just strategic values
        values_data = batch['values'][0]  # Remove pmap dimension [T, B] or [T, B, 1]
        
        # Flatten values to match rewards shape
        if len(values_data.shape) == 3 and values_data.shape[-1] == 1:
            strategic_values = values_data.squeeze(-1).flatten()
        elif len(values_data.shape) == 2:
            strategic_values = values_data.flatten()
        else:
            strategic_values = values_data.flatten()
        ...
        # Prepare targets
        # Since log_probs aren't being collected, use dummy values for now
        # TODO: Collect log_probs in actor for proper PPO implementation
        dummy_log_probs = jnp.zeros_like(advantages)
        ...
        # Define loss function with targets closed over
        def loss_fn(params):
    ...
    def _run(self):
        ...
        while not self.should_stop:
            ...
            update_count += 1
            self.global_step += self.config.batch_size * self.config.num_epochs
            
            # Log metrics
            if update_count % self.config.log_frequency == 0:
                metrics_np = jax.tree.map(lambda x: float(x[0]) if hasattr(x, 'shape') else float(x), metrics)
                
                # Log to TensorBoard
                if self.writer:
                    self.writer.add_scalar('loss/total', metrics_np.get('total_loss', 0), self.global_step)
                    
                    # Log agent-specific losses
                    if 'strategic' in metrics_np:
                        for key, value in metrics_np['strategic'].items():
                            self.writer.add_scalar(f'loss/strategic/{key}', value, self.global_step)
                    
                    for i in range(self.config.agent_config.num_operational_agents):
                        if f'operational_{i}' in metrics_np:
                            for key, value in metrics_np[f'operational_{i}'].items():
                                self.writer.add_scalar(f'loss/operational_{i}/{key}', value, self.global_step)
                    
                    if 'safety' in metrics_np:
                        for key, value in metrics_np['safety'].items():
                            self.writer.add_scalar(f'loss/safety/{key}', value, self.global_step)
                    
                    if 'coordination_loss' in metrics_np:
                        self.writer.add_scalar('loss/coordination', metrics_np['coordination_loss'], self.global_step)
...
def load_checkpoint(checkpoint_path: str, config: GridRLConfig):
    """Load checkpoint and return parameters and metadata"""
    print(f"Loading checkpoint from {checkpoint_path}...", flush=True)
    
    with open(checkpoint_path, 'rb') as f:
        checkpoint = pickle.load(f)
    
    print(f"Checkpoint loaded - Update count: {checkpoint.get('update_count', 0)}, "
          f"Global step: {checkpoint.get('global_step', 0)}", flush=True)
    
    return checkpoint


def train_grid_rl(args):
    ...
    if args.checkpoint:
        ...
        # Create training state with loaded parameters
        model = MultiAgentGridRL(config.agent_config)
        optimizer = optax.chain(
            optax.clip_by_global_norm(0.5),
            optax.adam(config.agent_config.learning_rate)
        )
        ...
        print(f"Resumed from checkpoint - Starting from update {update_count}", flush=True)
    else:
    ...
    learner.update_count = update_count
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Train Power Grid RL on TPU')
